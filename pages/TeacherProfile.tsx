  ...
  return (
    <div className="pt-24 min-h-screen pb-12 px-6 lg:px-12 max-w-4xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl lg:text-4xl font-display font-semibold text-white">Teacher Profile</h1>
        {saveSuccess && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="flex flex-col items-center text-center p-8 relative overflow-hidden">
          <div className="absolute top-0 right-0 size-32 bg-purple-500/10 blur-3xl rounded-full"></div>
          <div className="relative z-10 mb-4">
            <div className="size-32 rounded-full bg-purple-600 flex items-center justify-center text-5xl font-bold text-white shadow-xl border-4 border-white/10 overflow-hidden">
            ...
            <label htmlFor="teacher-avatar-upload" className="absolute bottom-0 right-0 size-9 bg-purple-600 rounded-full flex items-center justify-center cursor-pointer hover:bg-purple-500 transition-colors shadow-lg">
              {uploadingPhoto ? <Loader2 size={16} className="animate-spin text-white" /> : <Camera size={16} className="text-white" />}
              <input id="teacher-avatar-upload" type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} disabled={uploadingPhoto} />
            </label>
          </div>

          <h2 className="text-2xl font-semibold text-white mb-1 z-10">{formData.name || 'Teacher'}</h2>
        ...
        <div className="flex flex-col gap-6">
          <GlassCard className="p-8">
            ...
            <div className="space-y-5">
              <div>
                <span className="block text-xs font-bold text-zinc-400 uppercase mb-2">Display Name</span>
                {isEditing ? (
          ...
          <GlassCard className="p-6 border-amber-500/20 bg-amber-500/5">
            <h3 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Link2 size={18} className="text-amber-400" /> Your Class Code</h3>
            {classCode ? (
