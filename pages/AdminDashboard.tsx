...
import { ShieldCheck, Users, BookOpen, MessageSquare, Key, Activity, Search, Plus, Loader2, X, CheckCircle2, Clock, Eye, Trash2, UserPlus, BarChart3, RefreshCw, Star } from 'lucide-react';
    ...
    <div className="pt-24 pb-12 px-6 lg:px-12 max-w-7xl mx-auto min-h-screen">
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setSearch(''); }}
            className={`px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 whitespace-nowrap transition-all ${
              tab === t.id ? 'bg-white/10 text-white border border-white/15' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'
            }`}>
            {t.icon} {t.label}
              ...
              {activities.slice(0, 20).map((event, i) => (
                <div key={event.id || i} className="flex items-start gap-4 p-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white"><span className="font-bold">{event.actorName || event.actorEmail || 'User'}</span> — <span className="text-slate-400">{event.type?.replace(/_/g, ' ')}</span></p>
                    <p className="text-xs text-slate-500">{event.timestamp ? new Date(event.timestamp).toLocaleString() : ''}</p>
                  </div>
                  <span className="text-xs text-slate-500 font-mono shrink-0">{event.actorRole}</span>
                </div>
              ))}
              {activities.length === 0 && <div className="p-12 text-center text-slate-500">No activity yet</div>}
            </div>
          </GlassCard>
        </div>
      )}
                ...
                <tbody>
                  {filteredTeachers.map(t => (
                    <tr key={t.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-400 font-mono text-xs">{t.loginId || t.email}</td>
                      <td className="px-4 py-3 text-emerald-400 font-mono text-xs">{t.teacherId || '-'}</td>
                      <td className="px-4 py-3 text-slate-400">{t.institution || '-'}</td>
                      <td className="px-4 py-3 font-mono text-amber-400 font-bold tracking-widest">{t.classCode || t.class_code || '-'}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs">{t.createdAt ? new Date(t.createdAt).toLocaleDateString() : '-'}</td>
                  ...
                  {filteredTeachers.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-12 text-center text-slate-500">No teachers found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {filteredStudents.map(s => {
                    ...
                    return (
                      <tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-4 py-3 text-orange-400 font-bold">🔥 {s.streak || 0}</td>
                        <td className="px-4 py-3">
                          <button onClick={() => {
                  ...
                  {filteredStudents.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-12 text-center text-slate-500">No students found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                <tbody>
                  {feedbacks.map(fb => (
                    <tr key={fb.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-400 capitalize">{fb.category}</td>
                      <td className="px-4 py-3 text-amber-400">{'⭐'.repeat(fb.rating || 0)}</td>
                      <td className="px-4 py-3 text-slate-300 max-w-xs truncate" title={fb.message}>{fb.message}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs">{fb.submittedAt ? new Date(fb.submittedAt).toLocaleDateString() : ''}</td>
                      <td className="px-4 py-3">
                  ...
                  {feedbacks.length === 0 && (
                    <tr><td colSpan={8} className="px-4 py-12 text-center text-slate-500">No feedback received yet</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {tab === 'resets' && (
        <div className="space-y-4">
          {resets.length === 0 ? (
            <GlassCard className="p-12 text-center"><Key size={40} className="mx-auto text-slate-600 mb-4" /><p className="text-slate-400">No pending password reset requests.</p></GlassCard>
          ) : resets.map(req => (
            <GlassCard key={req.id} className="p-6 border-l-4 border-amber-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-400">{req.teacherId} — {req.failedAttempts} failed attempts</p>
                  <p className="text-xs text-slate-500">{req.requestedAt ? new Date(req.requestedAt).toLocaleString() : ''}</p>
                </div>
      ...
      {tab === 'activity' && (
        <GlassCard className="p-0 overflow-hidden">
          <div className="max-h-[600px] overflow-y-auto">
              ...
              <div key={event.id || i} className="flex items-start gap-4 p-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {event.metadata && Object.keys(event.metadata).length > 0 && (
                    <p className="text-xs text-slate-500 mt-0.5">{JSON.stringify(event.metadata).slice(0, 120)}</p>
                  )}
                  <p className="text-xs text-slate-500">{event.timestamp ? new Date(event.timestamp).toLocaleString() : ''}</p>
                </div>
            ...
            {activities.length === 0 && <div className="p-12 text-center text-slate-500">No activity recorded yet</div>}
          </div>
        </GlassCard>
      )}
      ...
      {showCreateTeacher && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl relative">
            <button onClick={() => setShowCreateTeacher(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X size={20} /></button>
            ...
            <form onSubmit={handleCreateTeacher} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Login ID</label>
                ...
                <p className="text-xs text-slate-500 mt-1">Must end with <code className="text-purple-400">@tchr.e-prayog</code></p>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Temporary Password</label>
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setShowCreateTeacher(false)} className="flex-1 py-3 rounded-xl text-slate-400 font-bold hover:bg-white/5 transition-colors">Cancel</button>
      ...
      {showResetModal && resetTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <p className="text-sm text-slate-400 mb-4">Enter a new password. The teacher's next login will use this password.</p>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowResetModal(false)} className="flex-1 py-3 rounded-xl text-slate-400 font-bold hover:bg-white/5 transition-colors">Cancel</button>
